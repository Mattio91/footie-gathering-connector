...
export const HourSlots: React.FC<HourSlotsProps> = ({ hourSlots, weekDays, isMobile = false }) => {
  return (
    <>
      {hourSlots.map(hour => (
        <div key={hour} className={`grid ${isMobile ? 'grid-cols-4' : 'grid-cols-8'} border-b last:border-b-0`}>
          {/* Hour label */}
